...
def generate_batch_llm_prompt(batch_quality_infos, batch_size=5):
    ...
    prompt = f"""
## Background
This is a data quality assessment task for multiple book data sources. The data sources contain book information such as author, title, publisher, ISBN, and publication year.

## Data Sources to Evaluate
{''.join(batch_summary)}

## Evaluation Requirements
Based on the data quality information and your domain knowledge about book data sources (e.g., some well-known publishers, libraries, and academic institutions are more trustworthy), please provide an initial confidence level for each data source (only the numerical level, no reasoning needed):
...
1. First, analyze the source name to determine which authoritative institution it comes from (e.g., national libraries, reputable publishers, university libraries, legitimate booksellers). This accounts for 60% of the evaluation.
2. Missing ratios of key attributes (author, title) account for 25% of the evaluation.
3. Data volume (very small datasets reduce reliability) accounts for 15% of the evaluation.

Examples of authoritative institutions:
- National libraries, Library of Congress, etc.
- Reputable publishers: Penguin Random House, HarperCollins, Springer, Elsevier, etc.
- University libraries: Harvard, Cambridge, Stanford, MIT, etc.
- Legitimate booksellers: Amazon, Barnes & Noble, etc.
- Professional book databases: ISBN.org, WorldCat, etc.
"""
    return prompt

def save_results_to_file(results, output_file='book_data_source_confidence.json'):
...
def generate_single_llm_prompt(quality_info):
    ...
    prompt = f"""
Book data source: {quality_info['source_name']}
...
def main():
    """Main function"""
    directory_path = "/home/lwh/QueryFusion/data/dataset/book/raw_data"
    ...
    print("Starting batch analysis of book data source quality...")
    ...
    if results:
        ...
        print("\n=== Book Data Source Confidence Summary ===")
        for result in results:
