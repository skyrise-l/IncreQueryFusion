...
def load_raw_sources(raw_dir: str, key_column: str):
    ...
    for path in paths:
        ...
        if sid is None:
            ...
            continue
        if sid in sources:
            print(f"[WARN] Multiple files for sid={sid}. Keeping first: {sources[sid].filename}, ignoring {fname}")
            continue
        ...
        try:
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                for i, raw in enumerate(f, start=-1):
                    ...
                    data = {key_column.lower(): key_value, "line": i, "data": parts}
                    rows.append(data)
...
def main():
    ...
    ap.add_argument("--golden", required=True, help="Path to the book_golden.txt (entity ids per non-empty line; first column is key_column).")
    ...
    entity_ids = parse_golden(args.golden)
    ...
    path = "/home/lwh/QueryFusion/preprocessing/query_truth_no_evaluate_book.json"
    write_query_truth(path, query_truth)
    print(f"[INFO] Snapshot written to: {path}")

if __name__ == "__main__":
    main()


# python result_create_book.py --raw-data-dir /home/lwh/QueryFusion/data/dataset/book/raw_data  --log-file /home/lwh/QueryFusion/data/dataset/book/log_data.json  --golden /home/lwh/QueryFusion/data/raw_data/book/book_golden.txt --queryjson /home/lwh/QueryFusion/data/raw_data/book/query_truth.json --key-column isbn
