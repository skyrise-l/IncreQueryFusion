...
class flightDataProcessor:
    ...
    def load_flight_data(self) -> pd.DataFrame:
        """加载原始电影数据"""
        # 读取原始数据（无列名）
        flight_data = pd.read_table(
            self.config['flight_data_path'], 
            sep="\t", 
            header=None
        )
        return flight_data
    
    def get_top_sources(self, flight_data: pd.DataFrame, n_sources: int = 13) -> List[str]:
        """获取前N个数据源"""
        source_counts = flight_data.iloc[:, 0].value_counts()  # 第2列是source
    ...
    def extract_query_info(self, queries: List[Dict]) -> Dict[str, Set[str]]:
        ...
        with open("/home/lwh/QueryFusion/preprocessing/flight_evaluate_llm.json", "r", encoding="utf-8") as f:
        ...
        for q in queries:
            ...
            predicted_answers = query_truth_llm[questionId]["predicted_answer"]

            for value, predict in zip(predicted_answers, query_truth_llm[questionId]["predict"]):
                if predict == True:
                    directors.add(str(value).strip().lower())

            query_info[key] = directors
            
        return query_info
    
    def collect_global_query_records(self, flight_data: pd.DataFrame, top_sources: List[str], query_info: Dict[str, Set[str]]) -> Dict[str, Dict]:
        """
        收集所有数据源中与查询相关的记录
        ...
        for key, truth_directors in query_info.items():
            ...
            title_records = flight_data[flight_data[1] == key]
    ...
    def select_global_query_samples(self, global_records: Dict[str, Dict]) -> Set:
        ...
        for key, records_info in global_records.items():
            ...
            all_truth = set(truth_indices).union(non_truth_indices)
            
            # 随机选择一半的真值记录
            n_truth_select = max(1, (len(truth_indices) + 1) // 2)
            n_non_truth_select = max(1, (len(non_truth_indices) + 1) // 2)
            ...
            unselected_truth = all_truth - selected_truth - selected_non_truth
            # 添加到选中集合
    ...
    def save_data_file(self, df: pd.DataFrame, file_path: str, include_timestamp: bool = False, index = False):
        ...
        """
        columns_to_keep = ['Source', 'Flight', 'Scheduled departure time', 'Actual departure time',
                       'Departure gate', 'Scheduled arrival time', 'Actual arrival time', 'Arrival gate']
        # 创建目标数据框
        ...
        for _, row in temp_df.iterrows():
            ...
            for i, col_name in enumerate(columns_to_keep):
                if col_name != 'timestamp':
                    flight_data[col_name.lower()] = row[col_name]

            self.log_data_operation(
                op_type="insert",
                sid=sid,
                row_data=flight_data,
    ...
    def process_data(self) -> List[int]:
        ...
        # 2. 获取前13个数据源
        top_sources = self.get_top_sources(flight_data, self.config['n_sources'])
        ...
        # 4. 收集全局查询记录
        global_records = self.collect_global_query_records(flight_data, top_sources, query_info)
        print("收集了全局查询记录分布")
        ...
        for sid, source_name in enumerate(top_sources, 1):
            ...
            # 获取该数据源的所有数据
            source_data = flight_data[flight_data[0] == source_name].copy()
...
def main():
    # 配置参数
    config = {
        'root_dir': "/home/lwh/QueryFusion/data/dataset/flight",
        'base_data_dir': "/home/lwh/QueryFusion/data/dataset/flight/raw_data",
        'final_data_dir': "/home/lwh/QueryFusion/data/dataset/flight/final_data",
        'flight_data_path': "/home/lwh/QueryFusion/data/raw_data/flight/flight_cleaned.txt",
        'query_truth_path': "/home/lwh/QueryFusion/data/dataset/flight/query_truth.json",
        'log_path': "/home/lwh/QueryFusion/data/dataset/flight/log_data.json",
