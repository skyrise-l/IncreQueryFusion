...
class bookDataProcessor:
    ...
    def load_book_data(self) -> pd.DataFrame:
        """加载原始电影数据"""
        # 读取原始数据（无列名）
        book_data = pd.read_table(
            self.config['book_data_path'], 
            sep="\t", 
            header=None
        )
        return book_data
    
    def get_top_sources(self, book_data: pd.DataFrame, n_sources: int = 13) -> List[str]:
        """获取前N个数据源"""
        source_counts = book_data.iloc[:, 0].value_counts()  # 第2列是source
    ...
    def extract_query_info(self, queries: List[Dict]) -> Dict[str, Set[str]]:
        ...
        entity_ids = []
        with open("/home/lwh/QueryFusion/data/raw_data/book/book_golden.txt", "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                raw = line.rstrip("\n")
                if not raw.strip():
        ...
        for q, key in zip(queries, entity_ids):
            directors = {d.strip().lower() for d in q.get("answers", [])}
            query_info[key] = directors

        return query_info
    
    def collect_global_query_records(self, book_data: pd.DataFrame, top_sources: List[str], query_info: Dict[str, Set[str]]) -> Dict[str, Dict]:
        """
        收集所有数据源中与查询相关的记录
        ...
        for key, truth_directors in query_info.items():
            title_records = book_data[book_data[1] == key]
    ...
    def process_data(self) -> List[int]:
        ...
        # 2. 获取前13个数据源
        top_sources = self.get_top_sources(book_data, self.config['n_sources'])
        ...
        # 4. 收集全局查询记录
        global_records = self.collect_global_query_records(book_data, top_sources, query_info)
        print("收集了全局查询记录分布")
        ...
        for sid, source_name in enumerate(top_sources, 1):
            ...
            # 获取该数据源的所有数据
            source_data = book_data[book_data[0] == source_name].copy()
...
def main():
    # 配置参数
    config = {
        'root_dir': "/home/lwh/QueryFusion/data/dataset/book",
        'base_data_dir': "/home/lwh/QueryFusion/data/dataset/book/raw_data",
        'final_data_dir': "/home/lwh/QueryFusion/data/dataset/book/final_data",
        'book_data_path': "/home/lwh/QueryFusion/data/raw_data/book/book.txt",
        'query_truth_path': "/home/lwh/QueryFusion/data/dataset/book/query_truth.json",
        'log_path': "/home/lwh/QueryFusion/data/dataset/book/log_data.json",
    ...
    # 创建处理器实例
    processor = bookDataProcessor(config)
